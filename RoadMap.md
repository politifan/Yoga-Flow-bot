...
9) **Безопасность и настройка**
   ...
   - Проверка идемпотентности webhook (не менять статус повторно).
   - Обрабатывать падения внешних сервисов (retry/backoff или graceful degrade).
...
10) **Тестирование (ручное, без фреймворков)**
    ...
    - Простые echo-скрипты для проверки /health, link-token, payment_session, webhook идемпотентность.

11) **Документация и развёртывание**
    - Обновить README: запуск dev (uvicorn + bot), пример `.env`.
    - Swagger/Redoc от FastAPI.
    - Инструкции для прод-запуска без Docker: uvicorn/gunicorn + reverse proxy; настройка webhook бота или polling.
    - Инструкции по переключению платёжного провайдера (sandbox → prod) и конфигу env.
